...
\begin{equation*}
h_{ab}(q) = \begin{cases}
\frac{\sigma}{2} \left( \frac{1}{d_{ab}(q)} - \frac{1}{d_{influence}} \right)^2, & \text{if } d_{ab}(q) \le d_{influence} \\
0, & \text{if } d_{ab}(q) > d_{influence}
\end{cases}
\end{equation*}
...
\STATE Update filtered delay state $\tau_{filt}(t)$ via LPF:
$$\dot{\tau}_{filt}(t) = \frac{1}{T_f} (\tau(t) - \tau_{filt}(t))$$
\STATE Compute filtered reference trajectory:
$$x_{d,filt}(t) = x_d(t - \tau_{filt}(t))$$
\STATE Compute reference feedforward velocity:
$$v_{d,filt}(t) = v_d(t - \tau_{filt}(t))(1 - \dot{\tau}_{filt}(t))$$
\STATE Compute task-space tracking error $e(t) = [e_p^T(t), e_o^T(t), e_f^T(t)]^T$ using unit quaternion orientation error.
\STATE Compute analytical system Jacobian $J(q(t)) \in \mathbb{R}^{11 \times 25}$.
\STATE Calculate Damped Least-Squares pseudo-inverse:
$$J^{\dagger}_{DLS}(q(t)) = J^T(q(t))\left(J(q(t))J^T(q(t)) + \lambda^2 I_{11}\right)^{-1}$$
\STATE Query spatial culling interface (BVH/Hashing) for active capsule pairs ($d_{ab} \le d_{influence}$).
\STATE Compute gradient of secondary task $\nabla H(q(t))$ consisting of joint boundaries, capsule-based self-collisions, and posture synergy:
$$\nabla H(q(t)) = \nabla H_{lim}(q(t)) + \nabla H_{coll}(q(t)) + \nabla H_{post}(q(t))$$
\STATE Compute saturated null-space velocity:
$$\dot{q}_0(t) = \text{sat}_{v_{max}} \left( -\alpha \nabla H(q(t)) \right)$$
\STATE Calculate command joint velocity vector:
$$\dot{q}(t) = J^{\dagger}_{DLS}(q(t)) \left[ v_{d,filt}(t) + K_p e(t) + K_d e(t - \tau_{filt}(t)) \right] + \left( I_{25} - J^{\dagger}_{DLS}(q(t))J(q(t)) \right) \dot{q}_0(t)$$
\RETURN $\dot{q}(t)$
\end{algorithmic}
\end{algorithm}
