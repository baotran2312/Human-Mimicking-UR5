...
\textbf{Lemma 1 (Reciprocal Convexity Lemma \cite{ref_dexsim2real})}: Let $R \in \mathbb{R}^{m \times m}$ be a positive-definite symmetric matrix. If there exists a matrix $S \in \mathbb{R}^{m \times m}$ such that:
\begin{equation}
...
\textbf{Lemma 2 (Fenchel's/Young's Inequality \cite{ref_deltadorsal})}: For any vectors $x, y \in \mathbb{R}^m$ and any symmetric positive-definite matrix $Q \in \mathbb{R}^{m \times m}$, the following inequality holds:
\begin{equation}
