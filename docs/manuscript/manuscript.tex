...
\begin{itemize}
\item Formulate a true-and-noise delay model that isolates non-differentiable network jitter into a bounded spatial tracking offset, enabling smooth tracking using a filtered delay reference.
\item Prove local Input-to-State Stability (LISS) of the tracking error under time-varying delays using a Lyapunov-Krasovskii Functional (LKF) and Linear Matrix Inequalities (LMIs) derived via the reciprocal convexity lemma.
\item Integrate a Damped Least-Squares (DLS) Jacobian inverse with secondary null-space projection tasks to systematically handle kinematic singularities, joint limits, self-collisions, and human-like hand postures.
\end{itemize}
...
\hline
Li \textit{et al.} (2022) \cite{ref_cybernetics_armhand} & Arm-Hand System & 25 & None & None & Joint limits only \\
AnyTeleop (2023) \cite{ref_anyteleop} & Unified Arm-Hand & 26 & None & Kinematic heuristics only & Joint limits and collisions \\
Jing \textit{et al.} (2024) \cite{ref_tase_tele} & Bilateral Manipulator & 2 & Time-Varying & Lyapunov stability & None \\
Bertino \textit{et al.} (2022) \cite{ref_tcst_delay} & Robot Manipulator & 7 & Constant Input Delay & Delay-Adaptive Lyapunov & Heuristic joint limits \\
Zhang \textit{et al.} (2021) \cite{ref_tcyb_nn} & Bilateral Manipulator & 2 & Time-Varying & Neural Network Fixed-Time & None \\
Zheng \textit{et al.} (2025) \cite{ref_tmech_manip} & Robot Manipulator & 6 & Constant Input Delay & Constrained Finite-Time & Input \& joint constraints \\
Li \textit{et al.} (2025) \cite{ref_tcyb_event} & Master-Slave System & 3 & Time-Varying & Event-Triggered Lyapunov & None \\
Kastritsi \textit{et al.} (2024) \cite{ref_tro_rcm} & Surgical Robot & 6 & Time-Varying & Passive Bilateral Control & RCM and spatial constraints \\
Guo \textit{et al.} (2025) \cite{ref_tro_grasp} & Multi-Fingered Hand & 19 & None & Reinforcement learning heuristic & Posture synergy only \\
Chicaiza \textit{et al.} (2025) \cite{ref_access_dual} & Dual-Arm Mobilipator & 12 & Time-Varying & Coupled Rate-Position mapping & None \\
Gutierrez-Giles (2023) \cite{ref_tcst_delayed_force} & Bilateral Teleoperation & 3 & Time-Varying & Passive force coordination & None \\
\textbf{Proposed Scheme} & \textbf{Unified Arm-Hand} & \textbf{25} & \textbf{Time-Varying + Jitter} & \textbf{LKF-LMI Rigorous LISS} & \textbf{Limits, self-collision \& synergy} \\
\hline
\end{tabular}
\end{table*}

\section{Preliminaries}
In this section, we present several fundamental definitions and mathematical lemmas that are crucial for the stability analysis and control design in subsequent sections.

Throughout this paper, for a symmetric matrix $P$, $P > 0$ (respectively, $P \ge 0$) denotes that $P$ is positive-definite (respectively, positive-semidefinite). The minimum and maximum eigenvalues of $P$ are represented by $\lambda_{min}(P)$ and $\lambda_{max}(P)$, respectively. The Euclidean norm of a vector $x \in \mathbb{R}^m$ is denoted by $\|x\| = \sqrt{x^T x}$.

\textbf{Lemma 1 (Reciprocal Convexity Lemma \cite{ref_park_reciprocal})}: Let $R \in \mathbb{R}^{m \times m}$ be a positive-definite symmetric matrix. If there exists a matrix $S \in \mathbb{R}^{m \times m}$ such that:
\begin{equation}
\begin{bmatrix} R & S \\ S^T & R \end{bmatrix} \ge 0
\end{equation}
then, for any scalar $\alpha \in (0, 1)$ and any vectors $x_1, x_2 \in \mathbb{R}^m$, the following inequality holds:
\begin{equation}
\frac{1}{\alpha} x_1^T R x_1 + \frac{1}{1-\alpha} x_2^T R x_2 \ge \begin{bmatrix} x_1 \\ x_2 \end{bmatrix}^T \begin{bmatrix} R & S \\ S^T & R \end{bmatrix} \begin{bmatrix} x_1 \\ x_2 \end{bmatrix}
\end{equation}

\textbf{Lemma 2 (Fenchel's/Young's Inequality \cite{ref_boyd_convex})}: For any vectors $x, y \in \mathbb{R}^m$ and any symmetric positive-definite matrix $Q \in \mathbb{R}^{m \times m}$, the following inequality holds:
\begin{equation}
x^T y \le \frac{1}{2} x^T Q x + \frac{1}{2} y^T Q^{-1} y
\end{equation}
In particular, for any positive scalar $\epsilon > 0$, we have:
\begin{equation}
2 x^T y \le \epsilon x^T x + \frac{1}{\epsilon} y^T y
\end{equation}

\textbf{Lemma 3 (Jensen's Inequality \cite{ref_tcs_haptic})}: For any symmetric positive-definite matrix $R \in \mathbb{R}^{m \times m}$, a scalar $h > 0$, and a vector function $x: [t-h, t] \to \mathbb{R}^m$ such that the integration is well-defined, the following inequality holds:
\begin{equation}
-h \int_{t-h}^{t} \dot{x}^T(s) R \dot{x}(s) ds \le - [x(t) - x(t-h)]^T R [x(t) - x(t-h)]
\end{equation}
...
Using Lemma 2 (Young's Inequality), the bilinear perturbation term is bounded by:
\begin{equation}
...
\end{equation}
Applying Lemma 2 (Young's Inequality) to the cross-term $2 \phi_e^T(t) R d_t(t)$ yields:
\begin{equation}
\dot{e}^T(t) R \dot{e}(t) \le (1 + \epsilon_2) \phi_e^T(t) R \phi_e(t) + \left( 1 + \frac{1}{\epsilon_2} \right) d_t^T(t) R d_t(t)
\end{equation}
where $\epsilon_2 > 0$.

To handle the delay-dependent integral term, we partition the integration interval into two segments: $[t - \tau_m, t - \tau_{filt}(t)]$ and $[t - \tau_{filt}(t), t]$. Applying Lemma 3 (Jensen's Inequality) to each segment yields:
\begin{equation}
...
Let $\eta(t) = [e^T(t), e^T(t-\tau_{filt}(t)), e^T(t-\tau_m)]^T \in \mathbb{R}^{3m}$ be the augmented state vector. Applying Lemma 1 (Reciprocal Convexity Lemma), if there exists a matrix $S \in \mathbb{R}^{m \times m}$ such that $\begin{bmatrix} R & S \\ S^T & R \end{bmatrix} \ge 0$, the partitioned segments can be bounded. We obtain:
\begin{equation}
...
\bibitem{ref_boyd_convex}
S. Boyd and L. Vandenberghe, \textit{Convex Optimization}. Cambridge University Press, 2004.
\bibitem{ref_park_reciprocal}
P. Park, J. W. Ko, and C. Jeong, ``Reciprocally convex approach to stability of systems with time-varying delays,'' \textit{Automatica}, vol. 47, no. 1, pp. 235--238, 2011.
\end{thebibliography}
