...
• Vision-Based Teleoperation:
  ...
  + Learning policies (3D Diffusion \cite{ref_diffpolicy3d}, 3D Flow \cite{ref_flowdiff}) require smooth human demonstration datasets
  + Markerless trackers (AnyTeleop \cite{ref_anyteleop}, DexSim2Real \cite{ref_dexsim2real}) use egocentric camera views \cite{ref_egocentric3d, ref_deltadorsal}
• Latency & Packet Jitter Challenges:
  ...
  + Jitter creates non-differentiable delay trajectories, breaking standard feedforward terms
...
• Unified Joint Space Configuration $q(t) \in \mathbb{R}^{25}$:
  ...
  + $q_h(t) \in \mathbb{R}^{19}$: Custom multi-fingered hand active joints
• Task Space Pose $x(t) \in \mathcal{X}$:
  + Relative Euclidean finger opposition distances ($n_f=5$) used to prevent over-constraint
  + Task dimension $m = 3 + 3 + 5 = 11 \ll 25$ total joints (redundant system)
...
• Filtered Reference Trajectory $x_{d,filt}(t)$ (LPF output):
    x_d,filt(t) = x_d(t - τ_filt(t))
  + $\tau_{filt}(t)$ is continuous, differentiable, and $|\dot{\tau}_{filt}| \le d < 1$
...
• Singularity-Robust Damped Least-Squares (DLS) Law:
    𝑞̇(t) = J†_DLS [v_d,filt(t) + K_p e(t) + K_d e(t - τ_filt(t))] + (I - J†_DLS J) 𝑞̇_0(t)
...
[BLOCK DIAGRAM: x_d(t) -> [Filter/Delay] -> x_d,filt -> [CLIK Controller] -> 𝑞̇(t) -> [Robot System] -> x(t)]
...
• Augmented Tracking Error Dynamics:
    ė(t) = -K_p e(t) - K_d e(t - τ_filt(t)) + d_t(t) + ΔE(e_o)[K_p e(t) + K_d e(t - τ_filt(t))]
...
• Candidate Lyapunov-Krasovskii Functional (LKF):
    V(t) = e(t)^T P e(t) + \int_{t - \tau_{filt}(t)}^{t} e(s)^T Q e(s) ds + \tau_m \int_{-\tau_m}^{0} \int_{t + \theta}^{t} \dot{e}(s)^T R \dot{e}(s) ds d\theta
...
• Reciprocal Convexity Lemma (Park et al., 2011):
  ...
  + If LMI $\Omega_{ISS} = \Omega_1 + \Omega_2 - \Omega_3 > 0$ is satisfied, then:
    V̇(t) \le -λ_min(Ω_ISS) ||η(t)||^2 + γ ||d_t(t)||^2
...
• 1. Joint Limit Avoidance $H_{lim}(q)$:
    H_lim(q) = \sum ((q_i - q̄_i) / (q_i,max - q_i,min))^2
• 2. Nomimal Posture Synergy $H_{post}(q)$:
...
• 3. Real-Time Self-Collision Avoidance $H_{coll}(q)$:
  ...
  + Active only within capsule-pair influence distance $d_{inf}$:
    h_ab(q) = (σ/2) * (1/d_ab(q) - 1/d_inf)^2   (if d_ab \le d_inf)
...
• Gradient Saturation (Choke Velocity):
  ...
  + Bounded potential vector norm: ||𝑞̇_0(t)|| \le \sqrt{n} v_max
...
• [COLOR: Theorem: SVD-based Null-Space Leakage Bound] (Remark 2):
  ...
  + Using Singular Value Decomposition $J = U \Sigma V^T$:
    J (I - J†_DLS J) = U diag( σ_i \lambda^2 / (σ_i^2 + \lambda^2) ) V^T
  + Diagonal elements are bounded by $\lambda/2$ for all $\sigma_i$ (AM-GM)
...
• CLIK Execution Loop (Algorithm 1):
  ...
  + 3. Compute reference trajectory $x_{d,filt}(t)$ and error $e(t)$
