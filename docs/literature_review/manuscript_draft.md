...
**Assumption 3**: The measured latency consists of a true delay $\tau_{true}(t)$ satisfying Assumptions 1 and 2, and a high-frequency bounded measurement noise $n(t)$ representing non-differentiable network packet jitter:
$$\tau(t) = \tau_{true}(t) + n(t), \quad \|n(t)\| \le n_{max}$$

Because the raw delayed trajectory $x_d(t - \tau(t))$ contains the high-frequency non-differentiable jitter $n(t)$, directly feeding it into a continuous-time differential controller violates smoothness requirements. In practice, the reference signal is processed through a continuous-time low-pass filter (or jitter buffer). Thus, we define the **filtered reference trajectory** $x_{d,filt}(t)$ equivalent to evaluating the trajectory at the filtered delay $\tau_{filt}(t)$:
...
*   $K_d e(t - \tau_{filt}(t))$ is the discrete delay-feedback compensation term.
*   $v_{d,filt}(t) = \frac{d}{dt} x_d(t - \tau_{filt}(t)) = v_d(t - \tau_{filt}(t))(1 - \dot{\tau}_{filt}(t))$ is the analytical derivative of the filtered reference trajectory.
...
Let $\eta(t) = [e^T(t), e^T(t-\tau_{filt}(t)), e^T(t-\tau_m)]^T \in \mathbb{R}^{3m}$ be the augmented state vector. Applying the **reciprocal convexity lemma** (Park et al., 2011), if there exists a matrix $S \in \mathbb{R}^{m \times m}$ such that $\begin{bmatrix} R & S \\ S^T & R \end{bmatrix} \ge 0$, the partitioned segments can be bounded. We obtain:
